...
dfs = []
columns = ['date','players_name','value']
...
for turn in range(total_turns):
    ...
    for player in players.items():
        ...
        if turn != 0:
            ...
            dfs.append(pd.DataFrame([[date,players_name,int(players[player[0]]['balance'])]], columns= columns))
        else:
            dfs.append(pd.DataFrame([[date,players_name,players_current_balance]], columns= columns))
